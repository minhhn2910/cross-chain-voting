...
const event_abi = {
  ...
  VotingResult: {
    name: "VotingResult",
    type: "event",
    inputs: [{ indexed: false, name: "winner", type: "uint256" }],
  },
};

// ABI for the SendMessage event
const sendMessageEventSignature =
  web3_instances[0].eth.abi.encodeEventSignature(event_abi.SendMessage);
const receiveMessageEventSignature =
  web3_instances[0].eth.abi.encodeEventSignature(event_abi.RecvMessage);
const voteCastEventSignature = web3_instances[0].eth.abi.encodeEventSignature(
  event_abi.VoteCast
);
const VotingResultEventSignature =
  web3_instances[0].eth.abi.encodeEventSignature(event_abi.VotingResult);
        ...
        if (log.topics[0] === sendMessageEventSignature) {
          ...
          try {
            ...
            const tx = await receiver_contract.methods
              .recv_message(event.session_id, event.data)
              .send({
                from: accounts[0],
                gas: 2000000,
              });
